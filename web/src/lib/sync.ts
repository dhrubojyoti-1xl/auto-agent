...
export async function syncAllAccounts(trigger: string): Promise<SyncSummary[]> {
  const accounts = await listGmailAccounts();
  const out: SyncSummary[] = [];
  for (const acct of accounts) {
    out.push(await syncAccount(acct, trigger));
  }
  if (out.some(s => s.rowsImported > 0)) await rebuildAnalysisAfterSync();
  return out;
}
...
export async function syncAccount(account: GmailAccount, trigger: string): Promise<SyncSummary> {
  ...
  const [{ id: runId }] = await query<{ id: number }>(
    `insert into sync_runs (gmail_account_id, trigger) values ($1,$2) returning id`,
    [account.id, trigger]
  );
  ...
  try {
    ...
    const seen = await loadSeenMessageIds();
    ...
    for (const id of fresh) {
      ...
      try {
        ...
        if (!documents.length) {
          ...
          await recordNonReport(account.id, msg, 'No report table in the body or attachments');
          continue;
        }
        ...
        for (const doc of documents) {
          ...
          await upsertGmailDocument(account.id, msg, doc, result.reportId, result.status,
            result.tablesFound, result.rowsExtracted, 0, result.skippedIdempotent,
            result.rejected.length);

          const written = await insertTasks(result.accepted);
          ...
          await upsertGmailDocument(account.id, msg, doc, result.reportId, result.status,
            result.tablesFound, result.rowsExtracted, written, result.skippedIdempotent,
            result.rejected.length);
...
async function loadSeenMessageIds(): Promise<Set<string>> {
  const rows = await query<{ gmail_message_id: string }>(
    `select distinct gmail_message_id from documents where gmail_message_id is not null`);
  return new Set(rows.map(r => r.gmail_message_id));
}

async function recordNonReport(accountId: number, msg: GmailMessage, reason: string): Promise<void> {
  await query(
    `insert into documents (report_id, document_id, source, subject, sender, sender_domain,
       ...
       rows_skipped_idempotent, rows_rejected, error_message, gmail_account_id, gmail_message_id)
     values ($1,$2,'email',$3,$4,$5,$6,'NO_DATA',0,0,0,0,0,$7,$8,$9)
     on conflict (report_id) do nothing`,
    [`GM-${msg.id}`, `gmail:${msg.id}`, msg.subject.slice(0, 300), msg.from,
     senderDomain(msg.from), msg.date, reason, accountId, msg.id]
  );
}

async function upsertGmailDocument(
  accountId: number, msg: GmailMessage, doc: SourceDocument, reportId: string,
  status: string, tablesFound: number, rowsExtracted: number, rowsInserted: number,
  rowsSkipped: number, rowsRejected: number
): Promise<void> {
  await query(
    `insert into documents (report_id, document_id, source, subject, sender, sender_domain,
       received_at, processing_status, tables_found, rows_extracted, rows_inserted,
       rows_skipped_idempotent, rows_rejected, gmail_account_id, gmail_message_id,
       attachment_name, processed_at)
    ...
    [reportId, doc.documentId, doc.attachmentName ? 'attachment' : 'email',
     ...
     accountId, msg.id, doc.attachmentName || null]
  );
}
...
export async function rebuildAnalysisAfterSync(): Promise<void> {
  const cfg = engineConfig();
